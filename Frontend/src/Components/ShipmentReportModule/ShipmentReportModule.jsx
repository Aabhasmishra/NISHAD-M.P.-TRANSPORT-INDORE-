...
const ShipmentReportModule = ({ isLightMode, modeOfView }) => {
  ...
  const isPPR = modeOfView === 'PPR';
  const isToday = modeOfView === 'TODAY';   // NEW
  ...
  useEffect(() => {
    const fetchRecords = async () => {
      ...
      try {
        ...
        if (isBR || isToday) {
          // BR and Today: all records (filter later)
          filtered = filtered;
        } else if (isPBR) {
          // PBR: paid !== 0
  ...
  const filteredRecords = useMemo(() => {
    ...
    if (isPPR) {
      ...
      start.setHours(7, 0, 0, 0); // 7 AM today
      if (now < start) {
        start.setDate(start.getDate() - 1); // before 7 AM → yesterday 7 AM
      }
      ...
      base = records.filter(rec => (parseFloat(rec.paid) || 0) !== 0);
    } else if (isBR) {
      base = records;
    } else {
      // OSR
      base = records.filter(rec => String(rec.challan_status).toUpperCase().trim() === 'NOT SHIPPED');
    }

    // 2. Apply month/custom range for PBR, BR, PPR (skip for Today and OSR)
    if (isToday || (!isPBR && !isBR && !isPPR)) {
      return base;
    }
    ...
    if ((isPBR || isPPR) && isCustomRange && !(customStartDate && customEndDate)) {
      return base.filter(rec => getMonthYear(rec.date) === previousMonth);
    }

    const active = selectedMonth;
    if (!active || active === 'custom') return base;
    ...
    return base.filter(rec => getMonthYear(rec.date) === active);
  }, [isPBR, isBR, isPPR, isToday, records, isCustomRange, customStartDate, customEndDate, selectedMonth, previousMonth, availablePeriods]);
  ...
  const processedRecords = useMemo(() => {
    ...
    let processed = source.map((rec, idx) => {
      ...
      return {
        ...
        amountCollected: parseFloat(rec.amount_collected) || 0,
  ...
  const downloadExcel = async () => {
    ...
    else if (isToday) sheetName = "Today's Booking";
    ...
    else if (isToday) titleText = "Today's Booking";
    else titleText = 'Outstanding Shipment Report';

    const colCount = isPPR ? 12 : 11;
    ...
    titleCell.fill = {
      type: 'pattern',
      pattern: 'solid',
      fgColor: { argb: isBR ? 'FF0F766E' : isPPR ? 'FF8B5CF6' : isToday ? 'FFD97706' : 'FF4F46E5' },
    };
    ...
    else if (isToday) fileName = `Todays_Booking_${formattedDate}.xlsx`;
  ...
  const activeFilterLabel = useMemo(() => {
    if (isToday) {
      ...
      return `Today: ${fmt(start)} → ${fmt(now)}`;
    }
  ...
  }, [isToday, isPBR, isBR, isPPR, isCustomRange, customStartDate, customEndDate, selectedMonth, availablePeriods]);

  // ── Render ─────────────────────────────────────────────────────────────────
  const showFilter = (isPBR || isBR || isPPR) && !isToday;
    ...
    <div className={`challan-main-container ${isLightMode ? 'light-mode' : 'dark-mode'}`} style={{ maxWidth: '1350px', paddingTop: '10px' }}>
      ...
      >
        ...
        <h2 className={`challan-title ${isLightMode ? 'light-mode' : 'dark-mode'}`} style={{ margin: 0 }}>
          {isBR ? 'Booking Register' : isPBR ? 'Paid Builty Report' : isPPR ? 'Payment Status Report' : isToday ? "Today's Booking" : 'Outstanding Shipment Report'}
        </h2>
      ...
      {!isLoading && !error && (
        <div className="challan-table-container" style={{ marginBottom: '0', overflowX: 'auto' }}>
          ...
          {/* Notes */}
          {!isPBR && !isBR && !isPPR && !isToday && records.length > 0 && (
          ...
          {isToday && processedRecords.length > 0 && (
            <div className={`challan-table-total-row ${isLightMode ? 'light-mode' : 'dark-mode'}`} style={{ fontWeight: 'inherit', marginTop: '10px', marginBottom: '15px', display: 'flex', justifyContent: 'start', padding: '10px' }}>
              <b style={{ marginRight: '4px', marginLeft: '4px' }}>Note:</b> This report shows all builties created from 7 AM yesterday to now.
            </div>
          )}
        </div>
      )}
