...
const TransactionHistory = ({ isLightMode, consignorName, consigneeName, onClose }) => {
  ...
  useEffect(() => {
    const fetchCustomerNames = async () => {
      try {
        const response = await fetch('http://localhost:3000/api/customers/all-names');
  ...
  const fetchTransportRecords = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/api/transport-records/history?consignor=${encodeURIComponent(consignor)}&consignee=${encodeURIComponent(consignee)}`
      );
      const data = await response.json();
